...
class SemanticFixer:
    """
    Correcteur sémantique post-décompilation.
    Corrige les artefacts typiques produits par le décompilateur :
      - Paramètres fantômes dans __init__ (variables locales promues en params)
      - Signatures de méthodes avec variables locales en params
      - Appels doubles comme perf_counter()()
      - None utilisé comme callable ou objet
      - Structures ctypes manquantes
      - Expressions invalides comme `TYPE % N`
    """
    ...
    def fix(self) -> str:
        lines = self.code.splitlines()
        lines = self._fix_init_phantom_params(lines)
        lines = self._fix_method_phantom_params(lines)
        lines = self._remove_injected_ctypes_params(lines)
    ...
    def _get_block(lines: List[str], start: int) -> List[int]:
        """Return indices of all lines that belong to the block starting at `start`."""
    ...
    # ------------------------------------------------------------------ fix 1: phantom params in __init__
    def _fix_init_phantom_params(self, lines: List[str]) -> List[str]:
        """
        Si __init__(self, cps, X, Y, Z) et que dans le corps on trouve
        X = <constante> AVANT toute utilisation de X comme valeur (rhs),
        alors X, Y, Z ne sont pas de vrais paramètres.
        On les retire de la signature et on laisse les assignations dans le corps.
        """
        result = list(lines)
        i = 0
        while i < len(result):
            line = result[i]
            m = re.match(r'^(\s*)def\s+__init__\s*\(self\s*,([^)]+)\)\s*:', line)
            if not m:
                i += 1
                continue
            ind = m.group(1)
            ...
            for param in raw_params:
                ...
                if pname == 'self':
                    continue
                # Scan all body lines: does pname get assigned a literal BEFORE being used as a value?
                first_assign_literal = False
                first_assign_idx = None
                first_use_as_value_idx = None

                for li, bl in enumerate(body_lines):
                    bs = bl.strip()
                    if not bs or bs.startswith('#'):
                        continue
                    assign_m = re.match(rf'^{re.escape(pname)}\s*=\s*(.+)', bs)
                    if assign_m and first_assign_idx is None:
                        rhs = assign_m.group(1).strip()
                        is_literal = bool(re.match(r'^(-?\d+(\.\d+)?|None|True|False|"[^"]*"|\'[^\']*\')$', rhs))
                        first_assign_literal = is_literal
                        first_assign_idx = li
                        continue
                    # Check if pname appears as a value (rhs, argument, etc.)
                    # but NOT as a lhs assignment
                    if first_use_as_value_idx is None:
                        # pname appears anywhere in the line NOT as lhs
                        if re.search(rf'\b{re.escape(pname)}\b', bs) and not re.match(rf'^{re.escape(pname)}\s*=', bs):
                            first_use_as_value_idx = li

                # It's phantom if it's assigned a literal before any use as a value
                if first_assign_literal and first_assign_idx is not None:
                    if first_use_as_value_idx is None or first_assign_idx < first_use_as_value_idx:
                        phantom.add(pname)

            if phantom:
                kept = [p for p in raw_params if p.split(':')[0].strip() not in phantom]
                new_sig = f"{ind}def __init__(self{', ' + ', '.join(kept) if kept else ''}):"
                result[i] = new_sig

            i += 1
        return result

    # ------------------------------------------------------------------ fix 1b: remove ctypes class params
    def _remove_injected_ctypes_params(self, lines: List[str]) -> List[str]:
        """Remove params that match injected ctypes structures (INPUT, MOUSEINPUT, etc.)"""
        INJECTED = {'INPUT', 'MOUSEINPUT', '_INPUT_UNION',
        ...
        for i, line in enumerate(result):
            ...
            kept = [p for p in params if p.split(':')[0].strip() not in INJECTED]
            if len(kept) < len(params):
                result[i] = f"{ind}def {fname}(self{', ' + ', '.join(kept) if kept else ''}):"
        return result

    # ------------------------------------------------------------------ fix 2: phantom params in regular methods
    def _fix_method_phantom_params(self, lines: List[str]) -> List[str]:
        """
        Si def run(self, x, y): et dans le corps on a
        x = expr AVANT toute utilisation de x comme valeur, x est un artefact.
        On retire ces params de la signature.
        """
        result = list(lines)
        i = 0
        while i < len(result):
            line = result[i]
            m = re.match(r'^(\s*)def\s+(\w+)\s*\(self\s*,([^)]+)\)\s*:', line)
            if not m or m.group(2) in ('__init__',):
                i += 1
                continue
            ind = m.group(1)
            ...
            for param in raw_params:
                ...
                for li, bl in enumerate(body_lines):
                    ...
                    if assign_m and first_assign_idx is None:
                        rhs = assign_m.group(1).strip()
                        # Phantom if pname doesn't appear on the RHS of its first assignment
                        if pname not in rhs:
                            first_assign_idx = li
                        continue
                    if first_use_as_value_idx is None:
            ...
            if phantom:
                kept = [p for p in raw_params if p.split(':')[0].strip() not in phantom]
                new_sig = f"{ind}def {method_name}(self{', ' + ', '.join(kept) if kept else ''}):"
                result[i] = new_sig
            i += 1
        return result

    # ------------------------------------------------------------------ fix 3: double calls foo()()
    def _fix_double_calls(self, lines: List[str]) -> List[str]:
        """Replace perf_counter()() and similar double-call artifacts."""
        result = []
        for line in lines:
            # e.g. time.perf_counter()()  →  time.perf_counter()
            fixed = re.sub(r'(\w[\w.]*\([^)]*\))\(\)', r'\1', line)
            result.append(fixed)
        return result

    # ------------------------------------------------------------------ fix 4: None used as callable
    def _fix_none_calls(self, lines: List[str]) -> List[str]:
        """Remove or comment lines where None is used as a function call."""
        result = []
        for line in lines:
            stripped = line.strip()
            ind = self._indent(line)
            # None(args) as a statement
            if re.match(r'^None\s*\(', stripped) and not stripped.startswith('#'):
                result.append(f'{ind}# TODO (decompile): {stripped}')
            # None.method(...) 
            elif re.match(r'^None\.\w+', stripped) and not stripped.startswith('#'):
                result.append(f'{ind}# TODO (decompile): {stripped}')
            else:
                result.append(line)
        return result

    # ------------------------------------------------------------------ fix 5: TYPE % N (ctypes array artifact)
    def _fix_type_modulo(self, lines: List[str]) -> List[str]:
        """
        Replace `(TYPE % N)(...)` with `(TYPE * N)(...)` — ctypes array artifact.
        The decompiler emits BINARY_OP % instead of * for ctypes array sizing.
        """
        result = []
        for line in lines:
            # (SomeName % 2)(args)  →  (SomeName * 2)(args)
            fixed = re.sub(r'\((\w+)\s*%\s*(\d+)\)', r'(\1 * \2)', line)
            result.append(fixed)
        return result

    # ------------------------------------------------------------------ fix 6: bare None statements
    def _fix_bare_none_statements(self, lines: List[str]) -> List[str]:
        """Remove bare `None` statements (no-op artifact)."""
        result = []
        for line in lines:
            stripped = line.strip()
            if stripped == 'None':
    ...
    # ------------------------------------------------------------------ fix 7: add missing ctypes structures
    def _add_missing_ctypes_structures(self, lines: List[str]) -> List[str]:
        """
        Detect references to MOUSEINPUT / INPUT ctypes structures and inject
        proper definitions if they're missing from the code.
        """
        code_str = '\n'.join(lines)
        additions = []
        ...
        # Insert additions after the import block (first non-import, non-empty line)
        insert_at = 0
        for idx, line in enumerate(lines):
            stripped = line.strip()
        ...
        new_lines = lines[:insert_at] + ''.join(additions).splitlines() + lines[insert_at:]
        return new_lines
