...
class SemanticFixer:
    ...
    def fix(self) -> str:
        ...
        lines = self._fix_tuple_index_calls(lines)
        lines = self._clean_invalid_globals(lines)
        lines = self._fix_all_phantom_params(lines)
        lines = self._fix_missing_params_from_callsites(lines)
        lines = self._clean_invalid_globals(lines)
        ...
        lines = self._resolve_anon_vars(lines)
        lines = self._wrap_main_guard(lines)
        lines = self._inject_missing_globals(lines)
        lines = self._clean_invalid_globals(lines)
    ...
    def _fix_missing_params_from_callsites(self, lines: List[str]) -> List[str]:
        ...
        while i < len(result):
            ...
            for pos in range(n_defined, max_args):
                abs_pos = pos + (1 if has_self else 0)
    ...
    def _analyze_cross_function_globals(self, lines: List[str]) -> Tuple[Set[str], Dict[str, Set[str]]]:
        ...
        needs_global_in: Dict[str, Set[str]] = {}
        undeclared_module_vars: Set[str] = set()
